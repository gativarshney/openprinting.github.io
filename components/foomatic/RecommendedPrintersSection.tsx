...
function getPrintersMap(): Promise<Map<string, PrinterSummary>> {
  if (!printersMapCache) {
    printersMapCache = fetch("/foomatic-db/printersMap.json")
      .then((r) => {
        if (!r.ok) throw new Error("printersMap fetch failed")
        return r.json() as Promise<PrintersMapData>
      })
      .then((data) => new Map(data.printers.map((p) => [p.id, p])))
      .catch((err) => {
...
function ConfidenceBadge({ score }: { score: number }) {
  ...
  return (
    <span className={`text-sm font-medium ${colorClass}`}>{pct}% match</span>
  )
}
...
export default function RecommendedPrintersSection({
  ...
  useEffect(() => {
    ...
    async function loadData() {
      setLoading(true)
      try {
        const [recsResponse, map] = await Promise.all([
        ...
        const recs: Recommendation[] = await recsResponse.json()
        setRecommendations(recs.slice(0, 3))
        setHasRecommendations(recs.length > 0)
      } catch {
        if (!cancelled) setHasRecommendations(false)
      } finally {
              ...
              <FoomaticCard key={recommendation.id} className="p-6">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    ...
                    {recommendation.sharedFeatures.length > 0 ? (
                      <div>
                        ...
                        <ul
                          ...
                          aria-label="Reasons this printer was recommended"
                        >
                  ...
                  <div className="flex shrink-0 flex-col items-end gap-3">
                    ...
                    <Link
                      href={`/foomatic/printer/${recommendation.id}`}
